            ...
            <aside class="lg:col-span-1">
                <div class="bg-white border border-sand/30 p-5 sticky top-28">
                    ...
                    @else
                        ...
                        <div class="text-center pb-5 border-b border-sand/30 mb-5">
                            <div
                                class="w-14 h-14 bg-sand/30 rounded-full flex items-center justify-center mx-auto mb-3">
                                <svg class="w-7 h-7 text-smoke" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                    ...
                    <nav class="space-y-1">
                        @if (!$isGuest)
                            ...
                            <button wire:click="$set('activeTab', 'orders')"
                                class="w-full flex items-center gap-3 px-3 py-2.5 font-body text-xs
                                       ...
                                       {{ $activeTab === 'orders' ? 'bg-blush-light text-ink' : 'text-smoke hover:text-ink hover:bg-blush-light/50' }}">
                        ...
                        <button wire:click="$set('activeTab', 'wishlist')"
                            class="w-full flex items-center gap-3 px-3 py-2.5 font-body text-xs
                                   tracking-widest uppercase transition-colors
                                   {{ $activeTab === 'wishlist' ? 'bg-blush-light text-ink' : 'text-smoke hover:text-ink hover:bg-blush-light/50' }}">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                            ...
                            @if ($wishlistCount > 0)
                                <span
                                    class="ml-auto bg-ink text-cream rounded-full w-4 h-4
                        ...
                        @if (!$isGuest)
                            ...
                            <button wire:click="$set('activeTab', 'addresses')"
                                class="w-full flex items-center gap-3 px-3 py-2.5 font-body text-xs
                                       tracking-widest uppercase transition-colors
                                       {{ $activeTab === 'addresses' ? 'bg-blush-light text-ink' : 'text-smoke hover:text-ink hover:bg-blush-light/50' }}">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                ...
                                <div class="bg-white border border-sand/30">
                                    <button wire:click="toggleOrder({{ $order->id }})"
                                        ...
                                        <div class="flex items-center gap-4">
                                            ...
                                            @endphp
                                            <span
                                                class="font-body text-xs px-3 py-1 rounded-full
                                        ...
                                        <div class="border-t border-sand/30" x-data="{ tab: 'items' }">
                                            <div class="flex border-b border-sand/30">
                                                <button @click="tab = 'items'"
                                                    :class="tab === 'items' ? 'border-b-2 border-ink text-ink' :
                                                        'text-smoke hover:text-ink'"
                                                    class="font-body text-xs tracking-widest uppercase px-6 py-3 transition-colors">
                                                    Ürünler
                                                </button>
                                                <button @click="tab = 'cargo'"
                                                    :class="tab === 'cargo' ? 'border-b-2 border-ink text-ink' :
                                                        'text-smoke hover:text-ink'"
                                                    class="font-body text-xs tracking-widest uppercase px-6 py-3 transition-colors">
                                                    Kargo Takip
                                                </button>
                                                <button @click="tab = 'address'"
                                                    :class="tab === 'address' ? 'border-b-2 border-ink text-ink' :
                                                        'text-smoke hover:text-ink'"
                                                    class="font-body text-xs tracking-widest uppercase px-6 py-3 transition-colors">
                                                    Teslimat
                                                </button>
                                                <button @click="tab = 'summary'"
                                                    :class="tab === 'summary' ? 'border-b-2 border-ink text-ink' :
                                                        'text-smoke hover:text-ink'"
                                                    class="font-body text-xs tracking-widest uppercase px-6 py-3 transition-colors">
                                                    Fiyat
                                                </button>
                                            ...
                                            <div x-show="tab === 'items'" class="p-5 space-y-3">
                                                @foreach ($order->items as $item)
                                                    <div class="flex gap-4">
                                                        <div
                                                            class="w-16 aspect-square bg-blush-light flex-shrink-0 overflow-hidden">
                                                        ...
                                                        <div class="flex-1">
                                                            ...
                                                            <div class="flex justify-between mt-1">
                                                                <span
                                                                    class="font-body text-xs text-smoke">{{ $item->quantity }}
                                                                    adet</span>
                                            ...
                                            <div x-show="tab === 'cargo'" class="p-5">
                                                @if ($order->cargo_tracking_number)
                                                    <div class="bg-blush-light/50 px-5 py-4 mb-6">
                                                        <div class="grid grid-cols-2 gap-4">
                                                            <div>
                                                                <p class="font-body text-xs text-smoke mb-1">Kargo
                                                                    Firması</p>
                                                                <p class="font-body text-sm font-medium text-ink">
                                                                    {{ $order->cargo_company ?? '—' }}</p>
                                                            </div>
                                                            <div>
                                                                <p class="font-body text-xs text-smoke mb-1">Takip No
                                                                </p>
                                                                <p class="font-body text-sm font-medium text-ink">
                                                                    {{ $order->cargo_tracking_number }}</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    @php
                                                        $trackingUrls = [
                                                            'Yurtiçi Kargo' =>
                                                                'https://www.yurticikargo.com/tr/online-islemler/gonderi-sorgula?code=' .
                                                                $order->cargo_tracking_number,
                                                            'Aras Kargo' =>
                                                                'https://kargotakip.araskargo.com.tr/?intlTrackId=' .
                                                                $order->cargo_tracking_number,
                                                            'MNG Kargo' =>
                                                                'https://www.mngkargo.com.tr/gonderi-takibi?trackingNo=' .
                                                                $order->cargo_tracking_number,
                                                            'PTT Kargo' =>
                                                                'https://gonderitakip.ptt.gov.tr/Track/Verify?q=' .
                                                                $order->cargo_tracking_number,
                                                            'Sürat Kargo' =>
                                                                'https://www.suratkargo.com.tr/KargoTakip/?TrackingNumber=' .
                                                                $order->cargo_tracking_number,
                                                            'HepsiJet' =>
                                                                'https://www.hepsijet.com/kargo-takip?trackingNumber=' .
                                                                $order->cargo_tracking_number,
                                                        ];
                                                ...
                                                @else
                                                    <div class="text-center py-10">
                                                        ...
                                                        <p class="font-body text-sm font-medium text-ink mb-1">Kargo
                                                            Bilgisi Henüz Yok</p>
                                                        <p class="font-body text-xs text-smoke">
                                                            @if ($order->status === 'pending')
                                                                Siparişiniz onay bekliyor.
                                                            @elseif($order->status === 'confirmed')
                                                                Siparişiniz onaylandı, hazırlanıyor.
                                                            @elseif($order->status === 'processing')
                                                                Siparişiniz hazırlanıyor.
                                                            @else
                                                                Kargo bilgisi eklendiğinde burada görünecek.
                                                            @endif
                                                        </p>
                                                    </div>
                                                @endif
                                            ...
                                            <div x-show="tab === 'address'" class="p-5">
                                                <div class="bg-blush-light/30 p-5 space-y-2">
                                                    <p
                                                        class="font-body text-xs tracking-widest uppercase text-smoke mb-3">
                                                        Teslimat Adresi</p>
                                                    <p class="font-body text-sm font-medium text-ink">
                                                        {{ $order->shipping_name }}</p>
                                                    <p class="font-body text-sm text-smoke">
                                            ...
                                            <div x-show="tab === 'summary'" class="p-5">
                                                <div class="max-w-xs space-y-2">
                                                    ...
                                                    @if ($order->discount_amount > 0)
                                                        <div
                                                            class="flex justify-between font-body text-xs text-green-600">
                                                            <span>İndirim</span>
                                                    ...
                                                    <div
                                                        class="flex justify-between font-body text-sm font-medium pt-3 border-t border-sand/30">
                                                        <span>Toplam</span>
                ...
                @if ($activeTab === 'wishlist')
                    ...
                    @if ($isGuest)
                        <div class="flex items-start gap-4 bg-blush-light/60 border border-sand/40 px-5 py-4 mb-6">
                            <svg class="w-5 h-5 text-smoke flex-shrink-0 mt-0.5" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                            ...
                            <div>
                                ...
                                <p class="font-body text-xs text-smoke leading-relaxed">
                                    Giriş yaparak favorilerinizi tüm cihazlarınızdan erişebilir, kaybetmeden
                                    saklayabilirsiniz.
                                </p>
                    ...
                    @if ($wishlistItems->isEmpty())
                        <div class="text-center py-20 bg-white border border-sand/30">
                            <svg class="w-12 h-12 text-sand mx-auto mb-4" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                ...
                @if ($activeTab === 'addresses' && !$isGuest)
                        ...
                        <div class="bg-white border border-sand/30 p-6 mb-6">
                            <div class="flex items-center justify-between mb-6">
                                ...
                                <button wire:click="closeAddressForm"
                                    class="text-smoke hover:text-ink transition-colors">
                            ...
                            <div class="grid sm:grid-cols-2 gap-4">
                                        ...
                                        @foreach (['Ev', 'İş', 'Diğer'] as $title)
                                            <button type="button"
                                                ...
                                                class="px-4 py-2 border font-body text-xs transition-all
                                                           {{ $addrTitle === $title ? 'bg-ink text-cream border-ink' : 'border-sand text-smoke hover:border-ink' }}">
                                                {{ $title }}
                                            </button>
                                        @endforeach
                                    </div>
                                ...
                                <div class="sm:col-span-2">
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input wire:model="addrIsDefault" type="checkbox"
                                            class="w-4 h-4 border-sand rounded" />
                                        <span class="font-body text-xs text-smoke">Varsayılan adres olarak
                                            ayarla</span>
                                    </label>
                                </div>
                    ...
                    @if ($addresses->isEmpty() && !$showAddressForm)
                        <div class="text-center py-20 bg-white border border-sand/30">
                            ...
                            <p class="font-body text-sm text-smoke mb-6">Alışverişi hızlandırmak için adresinizi
                                kaydedin.</p>
                            <button wire:click="openAddressForm()"
                                class="inline-block bg-ink text-cream font-body text-xs tracking-widest2
                                       uppercase px-8 py-3 hover:bg-smoke transition-colors">
                    ...
                    @else
                        <div class="grid sm:grid-cols-2 gap-4">
                            @foreach ($addresses as $addr)
                                <div
                                    class="bg-white border-2 p-5 relative
                                            {{ $addr->is_default ? 'border-ink' : 'border-sand/30' }}">
                                    @if ($addr->is_default)
                                        <span
                                            class="absolute top-3 right-3 font-body text-[10px]
                                    ...
                                    <div class="flex items-center gap-2 mb-3">
                                        <span
                                            class="font-body text-[10px] tracking-widest uppercase
                                                     bg-sand/30 text-smoke px-2 py-0.5">
                                            {{ $addr->title }}
                                        </span>
                                    </div>
                                    <p class="font-body text-sm font-medium text-ink">{{ $addr->first_name }}
                                        {{ $addr->last_name }}</p>
